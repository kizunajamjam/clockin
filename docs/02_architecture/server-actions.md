...
### `src/app/(protected)/shops/[shopId]/shifts/actions.ts`（オーナー側）

#### `createShift(prevState, formData)`
| フィールド | バリデーション |
|---|---|
| shop_id | 必須・オーナー所有確認（`getOwnerShop`） |
| staff_id / date / starts_at / ends_at | 必須 |
| break_minutes | 任意（デフォルト0） |

終了時刻が開始時刻以前なら翌日扱い（`+09:00`固定）。`shifts` に確定シフトを作成。

#### `updateShiftRequestStatus(formData)`
`shop_id`+`request_id`でスコープした `shift_requests` のステータス（pending/approved/rejected）を更新。
approved/rejected時は `sendPushToStaff` で対象スタッフへ通知（VAPID未設定時は自動スキップ）。

#### `createShiftFromRequest(formData)`
承認済み希望シフト（`shift_requests`）から確定シフト（`shifts`）を作成し、希望ステータスをapprovedにしてスタッフへ確定通知。希望時刻（start_time/end_time）が未指定の場合はエラー。

#### `deleteShift(formData)`
`shop_id`+`shift_id`でスコープして `shifts` を削除。

---

### `src/app/punch/shifts/actions.ts`（スタッフ側）

#### `upsertShiftRequest(formData)`
| フィールド | バリデーション |
|---|---|
| shop_id | 必須・所属確認（`getAuthedStaffForShop`） |
| date | 必須 |
| start_time / end_time / note | 任意 |

`(shop_id, staff_id, date)` の `onConflict` でupsert。ステータスは常に`pending`へリセット。

#### `deleteShiftRequest(formData)`
`request_id`+自分の`staff_id`でスコープして `shift_requests` を削除。

---

## カスタム給与項目

### `src/app/(protected)/shops/[shopId]/salary-items/actions.ts`（項目定義・オーナー側）

#### `createSalaryItem(prevState, formData)` / `updateSalaryItem(prevState, formData)`
| フィールド | バリデーション |
|---|---|
| shop_id | 必須・オーナー所有確認（`getOwnerShop`） |
| name | 必須 |
| type | count_unit / fixed / percentage / expense / time_unit のいずれか |
| unit_price | type が count_unit/time_unit/percentage の場合は必須・0以上 |

新規作成時は `sort_order` を末尾（現在の最大値+1）に設定。

#### `deleteSalaryItem(formData)`
`shop_id`でスコープして `salary_custom_items` を削除（`salary_custom_records` はFKの`ON DELETE CASCADE`で連動削除）。

---

### `src/app/(protected)/shops/[shopId]/payroll/[staffId]/actions.ts`（月次実績入力・オーナー側）

#### `upsertCustomRecords(prevState, formData)`
| フィールド | バリデーション |
|---|---|
| shop_id | 必須・オーナー所有確認（`getOwnerShop`） |
| staff_id | 必須・当該店舗への所属確認（`shop_staff`） |
| year_month | 'YYYY-MM' 形式（月は01〜12） |
| value_\<itemId\> | 任意・数値（空欄は0扱い） |

当該店舗の `salary_custom_items` に存在しない `item_id` は無視（他店舗項目のなりすまし防止）。
`(staff_id, item_id, year_month)` の `onConflict` でupsert。
